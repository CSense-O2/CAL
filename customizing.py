...
for drive in list(ascii_uppercase):
    for (path, dir, files) in os.walk(drive+':/'):
        if 'LOSTARK' in dir:
            customizing_path = path.replace('\\','/')+'/LOSTARK/EFGame/Customizing'
            break
...
if response.status_code == 200:
    ...
    try:
        ...
        sys.exit(0)
    if 최신버전 > 현재버전 :
        q1=msgbox.askquestion('최신 버전 발견','최신 버전 다운로드를 위해 링크가 열립니다.\n커마 경로 파일을 백업하시겠습니까?')
        if q1=='yes':
...
def find_file(num):
    file_dir = filedialog.askopenfile(initialdir="/", title=num+"번 관문 BGM 파일 선택", filetypes=(("CUS files", "*.cus"),("all files","*.*")))
    if filedialog.Open():
...
# pyinstaller --uac-admin --clean --noconsole --icon=icon.ico --add-data='.\customizing\MainFolder\*;MainFolder' .\customizing\customizing.py